...
struct Date {
    ...
    init(y:Int, m:Int, d:Int) {
        ...
        month = m
        day = d
        jdn = julianDayNumber(y, mo: m, dy: d)
        leapYear = isLeapYear(y)
        daysInMonth = numberOfDaysInMonth(y, mm:month)
        dayOfWeek = dayOfTheWeek(y, mm: m, dd: d)
    }
    ...
    init(iso8601:String) {
        ...
        jdn = julianDayNumber(y, mo: m, dy: d)
        leapYear = isLeapYear(y)
        daysInMonth = numberOfDaysInMonth(y, mm:m)
        dayOfWeek = dayOfTheWeek(y, mm: m, dd: d)
    }
...
//        var d:Int = day+1 // need to start at the next day and walk it forward until day of 
//          week is 1...6
//    }
...
struct Time {
    ...
    init(iso8601:String) {
        let parts:[String] = iso8601.componentsSeparatedByString(":")
        let hh:Int = parts[0].toInt()!
        let mm:Int = parts[1].toInt()!
        let ss:Int = parts[2].toInt()!
        let tz:Int = parts[3].toInt()!
        self.hours = hh
        self.minutes = mm
        self.seconds = ss
        self.tzOffset = tz
